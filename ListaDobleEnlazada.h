...
public:
    ...
    ~ListaDobleEnlazada();
    bool esVacia(); // Comprueba si la ListaDobleEnlazada est� vac�a
    uint longitud();
    virtual void mostrar(); // Muestra la ListaDobleEnlazada (metodo virtual)
    ...
    void agregarAlFinal(T v); // Agrega un elemento al final de la ListaDobleEnlazada

    // MODIFICAR
    void ordenarPorId(); // Ordena la ListaDobleEnlazada por el id

    void modificarInicial(T elem);  // Modifica el primer elemento de la ListaDobleEnlazada
    void modificarPos(T elem, uint pos); // Modifica un elemento de la ListaDobleEnlazada por su posición
    void modificarPorId(T elem, int id); // Modifica un elemento de la ListaDobleEnlazada por su id
    void modificarFinal(T elem); // Modifica el último elemento de la ListaDobleEnlazada

    // ELIMINAR
    void eliminarPorId(int id); // Elimina un elemento de la ListaDobleEnlazada por su id
    void eliminaInicial(); // Elimina el primer elemento de la ListaDobleEnlazada
    void eliminaPos(uint pos); // Elimina un elemento de la ListaDobleEnlazada por su posición
...
ListaDobleEnlazada<T, NADA>::~ListaDobleEnlazada()
{
    while (!esVacia())
    {
...
template <typename T, T NADA>
uint ListaDobleEnlazada<T, NADA>::longitud() {
    return lon;
}

template <typename T, T NADA>
bool ListaDobleEnlazada<T, NADA>::esVacia() {
    return lon == 0;
}
...
template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::agregarAlFinal(T v)
{
    this->agregaPos(v, lon);
}

template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::eliminarPorId(int id)
{
    if (lon == 0) return;
    if (ini->getId() == id) {
        eliminaInicial();
...
template<typename T, T NADA>
inline void ListaDobleEnlazada<T, NADA>::eliminaPos(uint pos)
{
    if (pos >= lon) return;
    if (pos == 0) {
        eliminaInicial();
...
template<typename T, T NADA>
inline void ListaDobleEnlazada<T, NADA>::eliminaFinal()
{
    eliminaPos(lon - 1);
}
...
template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::ordenarPorId()
{
    if (esVacia())
    {
        return;
...
template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::agregaPos(T elem, uint pos) {
    if (pos > lon) return;
    if (pos == 0) {
        agregarAlini(elem);
    }
...
template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::agregaPosConIdDesordenado(T elem, uint pos, int id) {
    if (pos > lon) return;
    if (pos == 0) {
        agregarAlini(elem);
    }
...
template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::agregaPorIdDesordenado(T elem, int id) {
    agregaPosConIdDesordenado(elem, lon, id);
}

template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::modificarInicial(T elem) {
    if (lon > 0) {
        ini->setDato(elem);
    }
}
...
template <typename T, T NADA>
void ListaDobleEnlazada<T, NADA>::modificarFinal(T elem) {
